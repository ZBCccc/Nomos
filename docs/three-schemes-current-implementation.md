...
当前已经统一成“头文件在 include，协议实现与实验实现在 src”的正常结构。
...
1. 它已经收敛成和 `Nomos` 当前简化实现同构的实验代码
   - 没有 owner / user / authorization 逻辑
   - 没有额外的多租户状态

2. 实现中的协议流程与 `Nomos` 对齐
   - 使用同一套 `F / F_p / Hash_H1`
   - 使用同构的 `update / genTokenSimplified / prepareSearch / search / decryptResults`

3. 当前保留的主要差异是工程定位
   - `MC-ODXT` 保持独立命名空间与独立类
   - 存储直接使用 map 容器
...
也就是说，第四章实验调用的是当前这套真实实现，而不是伪造数据。

当前实验计时已经和 `Nomos` 同构，包含：

- `McOdxtClient::genTokenSimplified(...)`
