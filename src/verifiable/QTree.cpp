...
void QTree::buildTree(const std::vector<bool>& bit_array) {
    ...
    while (current_level.size() > 1) {
        ...
        for (size_t i = 0; i < current_level.size(); i += 2) {
            ...
            internal->hash = hashInternal(internal->left->hash, internal->right->hash);
...
std::string QTree::generatePositiveProof(const std::vector<std::string>& addresses) {
    std::stringstream ss;
...
bool QTree::verifyPath(const std::string& address, bool bit_value,
    ...
    std::string current_hash = QTree(1 << proof.size()).hashLeaf(index, bit_value);
