...
int sampleBetaIndex(int ell) {
    ...
    const uint8_t limit =
        static_cast<uint8_t>(256 - (256 % ell));
    uint8_t sample = 0;
    do {
...
void McOdxtGatekeeper::computeF_p(
    bn_t result,
    const bn_t key,
...
UpdateMetadata McOdxtGatekeeper::update(
    OpType op,
    const std::string& id,
...
McOdxtClient::SearchRequest McOdxtClient::prepareSearch(
    ...
    const int m = static_cast<int>(
        std::min(token.bstag.size(), token.delta.size()));
    if (m == 0) {
        return req;
    }
...
std::vector<std::string> McOdxtClient::decryptResults(
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        ...
        for (size_t k = 0; k < result.sval.size(); ++k) {
            plaintext[k] = result.sval[k] ^ delta_bytes[k % delta_len];
        }
...
std::vector<SearchResultEntry> McOdxtServer::search(
    ...
    const int n = req.xtokenList.empty()
        ? 0
        : static_cast<int>(req.xtokenList[0].size()) + 1;
