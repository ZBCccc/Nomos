...
ClientSearchFixedW1Experiment::ClientSearchFixedW1Experiment()
    : dataset_(DatasetLoader::Dataset::None),
      ...
      repeat_count_(3),
      output_dir_("/Users/cyan/code/paper/Nomos/results/ch4/client_search_time_fixed_w1") {}
...
void ClientSearchFixedW1Experiment::run() {
    std::cout << "[ClientSearchFixedW1] Running Chapter 4 client search benchmark" << std::endl;
    ...
    for (size_t i = 0; i < datasets.size(); ++i) {
        ...
        std::cout << "\n[ClientSearchFixedW1] Dataset " << (i + 1) << "/" << datasets.size()
...
void ClientSearchFixedW1Experiment::setOutputDir(const std::string& output_dir) {
    if (!output_dir.empty()) {
        output_dir_ = output_dir;
    }
}

std::vector<DatasetLoader::Dataset> ClientSearchFixedW1Experiment::getDatasetsToRun() const {
    if (!run_all_datasets_ && dataset_ != DatasetLoader::Dataset::None) {
        return std::vector<DatasetLoader::Dataset>(1, dataset_);
    }

    return std::vector<DatasetLoader::Dataset>{
        DatasetLoader::Dataset::Crime,
...
ClientSearchFixedW1Experiment::buildDatasetSpec(DatasetLoader::Dataset dataset) const {
    DatasetLoader loader(dataset);
    if (!loader.load()) {
        throw std::runtime_error("Failed to load dataset: " + datasetToString(dataset));
    }

    const std::map<size_t, std::string> representatives =
        loader.getRepresentativeKeywordsByFrequency();
    if (representatives.empty()) {
        throw std::runtime_error("Dataset has no keyword frequencies: " + datasetToString(dataset));
    }
    ...
    for (std::map<size_t, std::string>::const_iterator it = representatives.begin();
    ...
    if (spec.w2_keyword.empty()) {
        throw std::runtime_error("Failed to select a distinct w2 keyword for dataset: " +
                                 datasetToString(dataset));
    }
...
void ClientSearchFixedW1Experiment::writeSchemeCsv(
    ...
    file << "dataset,scheme,upd_w1,upd_w2,client_search_time_ms,repeat,measurement_mode\n";
...
std::vector<double> ClientSearchFixedW1Experiment::runNomosSweep(
    ...
    for (size_t iteration = 0; iteration < repeat_count_; ++iteration) {
        ...
        for (size_t i = 0; i < kFixedUpdW1; ++i) {
            const UpdateMetadata meta = gatekeeper.update(OP_ADD, w1_doc_ids[i], spec.w1_keyword);
            server.update(meta);
        }
        ...
        for (size_t point = 0; point < spec.upd_w2_values.size(); ++point) {
            ...
            for (size_t next = current_w2_count; next < target_w2_count; ++next) {
                ...
                const UpdateMetadata meta = gatekeeper.update(OP_ADD, doc_id, spec.w2_keyword);
                server.update(meta);
            }
            ...
            const std::vector<SearchResultEntry> encrypted_results = server.search(request);
